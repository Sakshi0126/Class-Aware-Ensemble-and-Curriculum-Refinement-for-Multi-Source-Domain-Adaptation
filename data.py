...
# # New Office31Dataset class for .txt file loading
# class Office31Dataset(torch.utils.data.Dataset):
#     def __init__(self, root, image_list, transform=None, is_fix_trans=False):
#         self.root = root
#         self.transform = transform
#         self.is_fix_trans = is_fix_trans
#         # Load image list from .txt file
#         with open(os.path.join(root, image_list), 'r') as f:
#             self.samples = [(line.split()[0], int(line.split()[1])) for line in f.readlines()]  # e.g., ['back_pack/0001.jpg', 0]

#     def __getitem__(self, index):
#         path, target = self.samples[index]
#         sample = Image.open(os.path.join(self.root, path)).convert('RGB')
#         if self.transform is not None:
#             sample = self.transform(sample, index) if self.is_fix_trans else self.transform(sample)
...
#          transforms.Normalize(mean, std)])
#     '''
#     train_transform = TransformFixMatch(alpha=1-args.bt)
...
#     # Source datasets (webcam, dslr) using .txt files
#     train_src_sets = []
#     box_src_sets = []
#     for src in args.src:
#         tset = Office31Dataset(
#             root=args.data_path,
#             image_list=f"image_list/{src}_train.txt",
#             transform=srcTransform(alpha=1.0),
#             is_fix_trans=True
#         )
#         train_src_sets.append(tset)
#         box_src_sets.append(Office31Dataset(
#             root=args.data_path,
#             image_list=f"image_list/{src}_train.txt",
#             transform=box_transform
#         ))

#     # Target datasets (amazon) using .txt files
#     train_tar_set = Office31Dataset(
#         root=args.data_path,
#         image_list=f"image_list/{args.tar[0]}_train.txt",
#         transform=train_transform,
#         is_fix_trans=True
#     )
#     test_tar_set = Office31Dataset(
#         root=args.data_path,
#         image_list=f"image_list/{args.tar[0]}_test.txt",
#         transform=test_transform
#     )
#     box_tar_set = Office31Dataset(
#         root=args.data_path,
#         image_list=f"image_list/{args.tar[0]}_train.txt",
#         transform=box_transform
#     )
...
# New Office31Dataset class for .txt file loading
class Office31Dataset(torch.utils.data.Dataset):
    def __init__(self, root, image_list, transform=None, is_fix_trans=False, dataset_type="office31"):
        self.root = root
        self.transform = transform
        self.is_fix_trans = is_fix_trans
        ...
        # Load image list from .txt file
        with open(os.path.join(root, image_list), 'r') as f:
            self.samples = [(line.strip().split()[0], int(line.strip().split()[1])) for line in f.readlines()]  # e.g., ['back_pack/0001.jpg', 0]

    def __getitem__(self, index):
        path, target = self.samples[index]
        # Split path into domain and image path
        parts = path.split('/')
        domain = parts[0]
        img_path = '/'.join(parts[1:]) if len(parts) > 1 else path  # Handle cases where path is just the filename
        if self.dataset_type == "officehome":
            full_path = os.path.join(self.root, domain, img_path)  # e.g., ./data/Art/Drill/00014.jpg
        else:  # office31
            full_path = os.path.join(self.root, domain, img_path)  # e.g., ./data/webcam/images/printer/frame_0001.jpg
        sample = Image.open(full_path).convert('RGB')
        if self.transform is not None:
            sample = self.transform(sample, index) if self.is_fix_trans else self.transform(sample)
        return sample, target, index
...
def load_dataset(args):
    ...
    # Determine dataset type based on args.name
    dataset_type = args.name.lower()
    if dataset_type not in ['office31', 'officehome']:
        raise ValueError(f"Invalid dataset type in --name: '{args.name}'. Must be 'office31' or 'officehome'.")

    # Source datasets (webcam, dslr or Art, Clipart, etc.) using .txt files
    train_src_sets = []
    box_src_sets = []
    for src in args.src:
        tset = Office31Dataset(
    ...
    # Target datasets (amazon or Real_World) using .txt files
    train_tar_set = Office31Dataset(
        root=args.data_path,
        image_list=f"image_list/{args.tar[0]}_train.txt",
        transform=train_transform,
